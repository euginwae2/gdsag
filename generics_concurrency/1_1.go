...
func main() {
	...
	input1 := []float64{-5.0, -2.0, 4.0, 8.0}
	result4 := GenericMap[float64, float64](input1, func(f float64) float64 {
		return f * f
	})
	fmt.Println(result4)

	greaterThanfive := GenericFilter[int]([]int{4, 5, 6, 2, 20, 1, 7},
		func(i int) bool {
			return i > 5
		})
	fmt.Println(greaterThanfive)

	oddNumber := GenericFilter[int]([]int{4, 6, 5, 2, 20, 1, 7},
		func(i int) bool {
			return i%2 == 1
		})
	fmt.Println(oddNumber)

	lenthGreaterThan3 := GenericFilter[string]([]string{"hello", "or", "the", "maybe"}, func(s string) bool {
		return len(s) > 3
	})

	fmt.Println(lenthGreaterThan3)
}
